...
- Still Shifting = areas reworked 3+ times. Use the Churn data to identify these. Describe as the underlying capability or system being reworked ("the checkout flow", "the digest prompt", "the export pipeline", "the auth retry logic"), NEVER file names. UI areas, internal systems, prompts, parsers, schedulers all qualify — anything that kept getting touched. Observational, not alarming.
- Left Off = everything in progress when the session ended, visible or under the hood. Include ALL of them, not just one. Half-finished UI counts. So does a pipeline change that's wired up but not yet handling the edge case, a prompt rewrite that's in flight, a parser that's accepting the new format but not yet validating it. Apply the same capability test as Shipped — if the work is mid-flight and would change what the product can do once finished, it belongs here.
