    ...
    items: [
      {
        ...
        a: (
          <p>
            askscout is an AI-powered daily digest tool for developers. It reads your GitHub
            commits and diffs, then generates a clear summary of what you shipped each day. Think
            of it as a daily standup that writes itself, built for solo devs and vibe coders.
          </p>
        ),
      },
      {
      ...
      {
        q: "How do I track what I shipped this week?",
        a: (
          <p>
            askscout solves exactly this. Instead of scrolling git log or browsing GitHub
            activity, askscout summarizes your commits into a daily digest you can read, share,
            or paste into standup. It groups changes by what shipped, what changed, and what
            you left off, so you never lose track of progress across busy AI coding sessions.
          </p>
        ),
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: (
          <p>
            Yes. askscout never writes to your repo, only reads it. The codebase is open source
            under the MIT license, so you can audit exactly what it does. The web app uses GitHub
      ...
      },
      {
        ...
        a: (
          <p>
            ...
            web app currently only supports GitHub OAuth. GitLab and Bitbucket integrations on
            the web are on the roadmap but not yet shipped.
          </p>
        ),
      },
      {
        ...
        a: (
          <p>
            ...
            nuance or restate the same change twice. It is grounded in your actual commit
            messages and diffs, so it never invents work that did not happen. For day-to-day
            tracking, standup notes, and remembering what you shipped, accuracy is consistently
            high.
          </p>
        ),
      },
...
/* Plain-text Q&A pairs for JSON-LD FAQPage schema. Kept in parallel
   with the JSX answers above so the schema text mirrors what users
   read on the page (a Google guideline for FAQPage rich results).
   When you edit a JSX answer, update its plain twin here. */
export const FAQ_PLAIN: { q: string; a: string }[] = [
  {
    q: "What is askscout?",
    a: "askscout is an AI-powered daily digest tool for developers. It reads your GitHub commits and diffs, then generates a clear summary of what you shipped each day. Think of it as a daily standup that writes itself, built for solo devs and vibe coders.",
  },
  {
    q: "How does askscout work?",
    a: "Sign in with GitHub on the web, or run the askscout CLI in any local git repo. askscout reads your commit messages and diffs, sends them to Anthropic or OpenAI, and returns a structured digest of what shipped, what changed, and what you left off. The web app stores your digest history; the CLI runs entirely on your machine.",
  },
  {
    q: "Is askscout free?",
    a: "Yes. The askscout web app is free to use with a soft cap of 30 digests per day, which is plenty for daily standup or end-of-day reflection. The CLI is free open-source software. With the CLI you bring your own LLM API key, which costs roughly $0.001 to $0.003 per digest depending on repo size and provider.",
  },
  {
    q: "How do I install the askscout CLI?",
    a: "Install the CLI globally with npm install -g askscout, then run askscout --setup to add your Anthropic or OpenAI API key. After setup, run askscout in any git repo to generate a digest of recent commits. Full instructions are in the CLI docs.",
  },
  {
    q: "How do I track what I shipped this week?",
  ...
  {
    q: "Is askscout safe to use on my repo?",
    a: "Yes. askscout never writes to your repo, only reads it. The codebase is open source under the MIT license, so you can audit exactly what it does. The web app uses GitHub OAuth, which you can revoke any time at github.com/settings/applications.",
  },
  {
    q: "What data does askscout store?",
  ...
  {
    q: "What's the difference between the web app and the CLI?",
    a: "The askscout web app runs in your browser, signs in with GitHub, and stores your digest history under your account. The askscout CLI runs locally in any git repo, uses your own Anthropic or OpenAI API key, and stores nothing online. Same digest format, two surfaces, depending on whether you want a hosted history or a local-only workflow.",
  },
  {
    q: "Does askscout work with private repos?",
  ...
  {
    q: "Does askscout work with GitLab or Bitbucket?",
    a: "The askscout CLI works with any git repo regardless of host, including GitLab, Bitbucket, and self-hosted git servers, because it reads from your local clone. The web app currently only supports GitHub OAuth. GitLab and Bitbucket integrations on the web are on the roadmap but not yet shipped.",
  },
  {
    q: "How accurate is the askscout digest?",
    a: "The digest is generated by an LLM (Claude or GPT), so it can occasionally miss nuance or restate the same change twice. It is grounded in your actual commit messages and diffs, so it never invents work that did not happen. For day-to-day tracking, standup notes, and remembering what you shipped, accuracy is consistently high.",
  },
  {
    q: "Is there a usage limit?",
    a: "The askscout web app has a soft cap of 30 digests per day across your account, which covers daily standup and end-of-day reviews comfortably. The CLI has no askscout usage limit; the only limits are your LLM provider's rate limits and your own spend on Anthropic or OpenAI.",
  },
];
