 ...
 * inside a star ring and glow backdrop (<HeroStarfield />).
 *
 * The background used to be a 7 MB looping starfield video that
 * covered the whole hero, plus a 60% scrim to knock it back and a
 * mask to stop it banding at the section boundary. That whole
 * stack is gone: the atmosphere now lives in the gutter around
 * the digest frame instead of behind the text.
 *
 * Extracted from MarketingHome.tsx so future /dev/* compare
 * routes can render two instances side-by-side — visual
 * comparison happens against the real production hero treatment,
 * not a stripped-down mock.
 *
 * Server component (no "use client") because it embeds a server
 * action in the GitHub sign-in form. The interactive child pieces
 * (HeroStarfield, HeroGraphicMotion, HeroCardOpener, InstallChip)
 * are client components and run normally.
 */
export function HeroSection({ ctaDisabled = false }: Props = {}) {
  return (
    <section className="home-hero">
      {/* The bottom-fade stays: it still dissolves the graphic's
        hard bottom border into the page. Its other old job —
        masking the bg video — is gone with the video. */}
      ...
      <HeroStarfield>
        ...
        {/* Mobile-only static export of the digest graphic. The live
          ...
          flat PNG export of the Figma. Sits inside the starfield
          wrapper so the ring frames whichever of the two is visible. */}
