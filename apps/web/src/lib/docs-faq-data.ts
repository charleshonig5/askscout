/* Plain-text Q&A pairs for the docs page FAQ. Server-importable so
   ...
   snippets in Google search alongside the rendered tabs. Order
   mirrors the rendered tabs in <DocsFAQ />: Web app first (most
   common landing case), then CLI. Every question explicitly names
   which surface it applies to so readers landing on the wrong tab
